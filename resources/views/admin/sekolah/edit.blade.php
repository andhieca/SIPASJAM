<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight flex items-center gap-2">
            ...
            Edit Data Sekolah: {{ $sekolah->nama_sekolah }}
        </h2>
    </x-slot>

    <div class="py-12" x-data="sekolahEditModal">
                    ...
                    <form action="{{ route('sekolah.update', $sekolah->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="desa_id" required class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-pj-green-500 focus:ring focus:ring-pj-green-200">
                                    ...
                                    @foreach($desas as $desa)
                                        <option value="{{ $desa->id }}" {{ $sekolah->desa_id == $desa->id ? 'selected' : '' }}>{{ $desa->nama_desa }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">NPSN</label>
                                <input type="text" name="npsn" value="{{ $sekolah->npsn }}" placeholder="Contoh: 20201234" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-pj-green-500 focus:ring focus:ring-pj-green-200">
                            </div>
                            <div class="col-span-2">
                                <label class="block text-sm font-medium text-gray-700">Nama Sekolah *</label>
                                <input type="text" name="nama_sekolah" value="{{ $sekolah->nama_sekolah }}" required class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-pj-green-500 focus:ring focus:ring-pj-green-200">
                            </div>
                            <div class="col-span-2">
                                <label class="block text-sm font-medium text-gray-700">Alamat Sekolah</label>
                                <textarea name="alamat_sekolah" rows="2" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-pj-green-500 focus:ring focus:ring-pj-green-200">{{ $sekolah->alamat_sekolah }}</textarea>
                            </div>
                            <div class="col-span-2">
                                <label class="block text-sm font-medium text-gray-700">Koordinat Lokasi</label>
                                <input type="text" name="koordinat_lokasi" id="koordinat_lokasi" value="{{ $sekolah->koordinat_lokasi }}" @input="updateMapFromInput($event.target.value)" placeholder="Cth: -7.123, 107.123" class="mt-1 mb-3 block w-full rounded-lg border-gray-300 shadow-sm focus:border-pj-green-500 focus:ring focus:ring-pj-green-200">
                            ...
                            <div class="col-span-1 sm:col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-2">Foto Sekolah (Maksimal 3 Foto)</label>
                        ...
                        <div class="mt-8 flex justify-end gap-3 pt-6 border-t border-gray-100">
                            <a href="{{ route('sekolah.index') }}" class="inline-flex justify-center rounded-lg border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none sm:w-auto sm:text-sm">Batal</a>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('sekolahEditModal', () => ({
                map: null,
                marker: null,
                existingPhotos: @json($sekolah->foto_sekolah ?? []),
                photoPreviews: [],
                photoError: '',
                ...
                removePreviewPhoto(index) {
                    ...
                    const input = document.getElementById('foto_sekolah_input_edit');
                ...
                init() {
                    ...
                    const existingCoords = '{{ $sekolah->koordinat_lokasi }}';
